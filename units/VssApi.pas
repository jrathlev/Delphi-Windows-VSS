{ Delphi VssAPI interface
  ...
  see: Windows SDFK 7.0 - Include\vss.h, vsbackup.h , vswriter.h
    https://msdn.microsoft.com/de-de/library/windows/desktop/aa384629(v=vs.85).aspx
    https://msdn.microsoft.com/de-de/library/windows/desktop/aa384648(v=vs.85).aspx
...
function CreateVssBackupComponents (out ppBackup : IVssBackupComponents) : HResult; stdcall;
procedure VssFreeSnapshotProperties(pProp : PVssSnapshotProp); stdcall;

function ClusterIsPathOnSharedVolume(lpszPathName: PWideChar) : BOOL; winapi;
function ClusterGetVolumePathName(lpszFileName: PWideChar; lpszVolumePathName: PWideChar;
         cchBufferLength: DWORD): BOOL; winapi;
function ClusterGetVolumeNameForVolumeMountPoint(lpszVolumeMountPoint: PWideChar;
...
var
  ...
  FClusterGetVolumeNameForVolumeMountPoint : TClusterGetVolumeNameForVolumeMountPoint;
...
begin
  if assigned(FCreateVssBackupComponents) then Result:=FCreateVssBackupComponents(ppBackup);
  end;

procedure VssFreeSnapshotProperties (pProp : PVssSnapshotProp);
begin
  if assigned(FVssFreeSnapshotProperties) then FVssFreeSnapshotProperties(pProp);
  end;

function ClusterIsPathOnSharedVolume(lpszPathName: PWideChar) : BOOL;
begin
  if assigned(@FClusterIsPathOnSharedVolume) then Result:=FClusterIsPathOnSharedVolume(lpszPathName)
  else Result:=false;
  end;
...
begin
  if assigned(@FClusterGetVolumePathName) then
    Result:=FClusterGetVolumePathName(lpszFileName,lpszVolumePathName,cchBufferLength)
  else Result:=false;
  end;

function ClusterGetVolumeNameForVolumeMountPoint(lpszVolumeMountPoint: PWideChar;
         lpszVolumeName: PWideChar; cchBufferLength: DWORD): BOOL;
begin
  if assigned(@FClusterGetVolumePathName) then
    Result:=FClusterGetVolumeNameForVolumeMountPoint(lpszVolumeMountPoint,lpszVolumeName,cchBufferLength)
  else Result:=false;
  end;
...
initialization
  ...
  if VssApiHandle=0 then VssApiHandle:=LoadLibrary(VssApiDll);
  if VssApiHandle<>0 then begin     // following entries are not available on Windows XP
    @FCreateVssBackupComponents:=GetProcAddress(VssApiHandle,'CreateVssBackupComponentsInternal');
    @FVssFreeSnapshotProperties:=GetProcAddress(VssApiHandle,'VssFreeSnapshotPropertiesInternal');
    FIsVssAvailable:=assigned(FCreateVssBackupComponents) and assigned(FVssFreeSnapshotProperties);
    end
  else FIsVssAvailable:=false;
  ResApiHandle:=GetModuleHandle(ResApiDll);
  if ResApiHandle=0 then ResApiHandle:=LoadLibrary(ResApiDll);
  if ResApiHandle<>0 then begin     // only Windows Server 2008/12 R2 Enterprise, R2 Datacenter
    @FClusterIsPathOnSharedVolume:=GetProcAddress(ResApiHandle,'ClusterIsPathOnSharedVolume');
    @FClusterGetVolumePathName:=GetProcAddress(ResApiHandle,'ClusterGetVolumePathName');
    @FClusterGetVolumeNameForVolumeMountPoint:=GetProcAddress(ResApiHandle,'ClusterGetVolumeNameForVolumeMountPoint');
    end;
end.
